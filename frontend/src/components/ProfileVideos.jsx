...
function ProfileVideo() {
    ...
    useEffect(() => {
        async function fetchUserPosts() {
            try {
                const response = await fetch(apiUrl + 'u/' + userId+'/p/');
            ...
            <>
                <div className="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div className="text-muted small">
                         Showing {1*content.page}-{content.size*content.page} of {content.totalElements} total results
                    </div>
                    ...
                    <nav>
                        ...
                        {content.totalPages>6 && (
                            <ul className="pagination justify-content-center">
                                ...
                                {pages.map((p,i) => (
                                    <li className={`page-item ${p===content.page ? "active" : ""}`} key={i}>
                                        <a className={`page-link page-num ${p===content.page ? "active" : ""}`}
                                           href="#" data-value={p} onClick={(e) => {e.preventDefault();setP(e.currentTarget.dataset.value)}}>{p}</a>
                                    </li>
                                ))}
